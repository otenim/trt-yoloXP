...
namespace tensorrt_yolox {
  TrtYoloX::TrtYoloX(const std::string &model_path,
                     const std::string &precision, const int num_class,
    ...
    if (needs_output_decode_) {
      ...
      out_elem_num_per_batch_ =
          static_cast<int>(out_elem_num_ / batch_config[2]);
      out_prob_d_ = cuda_utils::make_unique<float[]>(out_elem_num_);
      out_prob_h_ = cuda_utils::make_unique_host<float[]>(
          out_elem_num_, cudaHostAllocPortable);
      ...
      input_d_ = cuda_utils::make_unique<float[]>(batch_config[2] * input_size);
      out_num_detections_d_ =
          cuda_utils::make_unique<int32_t[]>(batch_config[2]);
      out_boxes_d_ = cuda_utils::make_unique<float[]>(batch_config[2] *
                                                      max_detections_ * 4);
      out_scores_d_ =
  ...
  void TrtYoloX::initPreprocessBuffer(int width, int height) {
    ...
    if (use_gpu_preprocess_) {
      ...
      if (!image_buf_h_) {
        const float scale =
            std::min(input_width / width, input_height / height);
  ...
  void TrtYoloX::preprocess(const std::vector<cv::Mat> &images) {
    ...
    const auto chw_images =
        cv::dnn::blobFromImages(dst_images, norm_factor_, cv::Size(),
  ...
  void TrtYoloX::multiScalePreprocess(const cv::Mat &image,
    ...
    for (const auto &roi : rois) {
      ...
      const float scale =
          std::min(input_width / static_cast<float>(roi.width),
  ...
  bool TrtYoloX::feedforward(const std::vector<cv::Mat> &images,
    ...
    auto out_boxes =
        std::make_unique<float[]>(4 * batch_size * max_detections_);
    auto out_scores = std::make_unique<float[]>(batch_size * max_detections_);
    auto out_classes =
        std::make_unique<int32_t[]>(batch_size * max_detections_);
    ...
    CHECK_CUDA_ERROR(
        cudaMemcpyAsync(out_scores.get(), out_scores_d_.get(),
    ...
    for (size_t i = 0; i < batch_size; ++i) {
      ...
      for (size_t j = 0; j < num_detection; ++j) {
        ...
        object.x_offset =
            std::clamp(0, static_cast<int32_t>(x1), images[i].cols);
        object.y_offset =
            std::clamp(0, static_cast<int32_t>(y1), images[i].rows);
...
#pragma omp section
  ...
  cv::Mat TrtYoloX::getMaskImageGpu(float *d_prob, nvinfer1::Dims dims,
                                    int out_w, int out_h, int b) {
    // NCHW
    ...
    argmax_gpu((unsigned char *)argmax_buf_d_.get() + index, d_prob, out_w,
               out_h, width, height, classes, 1, *stream_);
  ...
  cv::Mat TrtYoloX::getColorizedMask(int index,
