...
namespace tensorrt_common {
  ...
  bool is_valid_precision_string(const std::string &precision) {
    if (std::find(valid_precisions.begin(), valid_precisions.end(),
                  precision) == valid_precisions.end()) {
...
// cspell: ignore asan
      ...
      // shared library on close so that asan can resolve the symbols correctly.
      flags |= RTLD_NODELETE;
#endif  // ENABLE_ASAN
  ...
  void TrtCommon::setup() {
    ...
    if (model_file_path_.extension() == ".engine") {
      ...
      if (build_config_->dla_core_id != -1) {
        ext = "DLA" + std::to_string(build_config_->dla_core_id) + "-" +
              calib_name + precision_;
    ...
    std::string ret =
        getLayerInformation(nvinfer1::LayerInformationFormat::kJSON);
  ...
  bool TrtCommon::loadEngine(const std::string &engine_file_path) {
    ...
    engine_ =
        TrtUniquePtr<nvinfer1::ICudaEngine>(runtime_->deserializeCudaEngine(
            reinterpret_cast<const void *>(engine_str.data()),
            engine_str.size()));
    return true;
  }
  ...
  void TrtCommon::printNetworkInfo(const std::string &onnx_file_path) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      if (layer_type == nvinfer1::LayerType::kCONVOLUTION) {
        ...
        int num_weights =
            (dim_in.d[1] / groups) * dim_out.d[1] * k_dims.d[0] * k_dims.d[1];
        float gflops = (2 * num_weights) *
                       (dim_in.d[3] / stride * dim_in.d[2] / stride / 1e9);
        ;
        total_gflops += gflops;
        total_params += num_weights;
        std::cout << "L" << i << " [conv " << k_dims.d[0] << "x" << k_dims.d[1]
                  << " (" << groups << ") " << "/" << s_dims.d[0] << "] "
                  << dim_in.d[3] << "x" << dim_in.d[2] << "x" << dim_in.d[1]
                  << " -> " << dim_out.d[3] << "x" << dim_out.d[2] << "x"
                  << dim_out.d[1];
        ...
        float gflops = dim_in.d[1] * dim_window.d[0] / dim_stride.d[0] *
                       dim_window.d[1] / dim_stride.d[1] * dim_in.d[2] *
                       dim_in.d[3] / 1e9;
        total_gflops += gflops;
    ...
    std::cout << "Total " << total_params / 1000.0 / 1000.0 << " M params"
              << std::endl;
    return;
  }
  ...
  bool TrtCommon::buildEngineFromOnnx(
    ...
    if (build_config_->dla_core_id != -1) {
      if (num_available_dla > 0) {
        std::cout << "###" << num_available_dla << " DLAs are supported! ###"
                  << std::endl;
      } else {
      ...
      for (int i = 0; i < num; i++) {
        ...
        if (build_config_->clip_value > 0.0) {
          std::cout << "Set max value for outputs : "
                    << build_config_->clip_value << "  " << name << std::endl;
    ...
    if (batch_config_.at(0) > 1 &&
      ...
      if (build_config_->profile_per_layer) {
        ...
        profile->setDimensions(
            ...
            nvinfer1::Dims4{batch_config_.at(0), input_channel, input_height,
                            input_width});
        profile->setDimensions(
            network->getInput(0)->getName(), nvinfer1::OptProfileSelector::kOPT,
            nvinfer1::Dims4{batch_config_.at(1), input_channel, input_height,
                            input_width});
        profile->setDimensions(
            network->getInput(0)->getName(), nvinfer1::OptProfileSelector::kMAX,
            nvinfer1::Dims4{batch_config_.at(2), input_channel, input_height,
                            input_width});
    ...
    8200
      ...
      //    assert((calibrator != nullptr) && "Invalid calibrator for INT8
      //    precision");
  ...
  bool TrtCommon::setBindingDimensions(const int32_t index,
                                       const nvinfer1::Dims &dimensions) const {
    return context_->setBindingDimensions(index, dimensions);
  }

  bool TrtCommon::enqueueV2(void **bindings, cudaStream_t stream,
    ...
    if (build_config_->profile_per_layer) {
      ...
      host_profiler_.reportLayerTime(
          "inference", std::chrono::duration<float, std::milli>(inference_end -
                                                                inference_start)
                           .count());
      return ret;
    } else {
  ...
  void TrtCommon::printProfiling() {
    ...
    std::cout << model_profiler_;
        ...
        NV_TENSOR_PATCH >=                                   \
    8200
  std::string TrtCommon::getLayerInformation(
