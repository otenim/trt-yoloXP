...
void write_prediction(std::string dumpPath, std::string filename,
                      ...
                      tensorrt_yolox::ObjectArray objects, int width,
                      int height) {
  ...
  for (const auto& object : objects) {
    ...
    std::string writing_text =
        format("%s %f %d %d %d %d", names[object.type].c_str(), object.score,
...
void write_label(std::string outputPath, std::string filename,
                 std::vector<std::string> names,
                 tensorrt_yolox::ObjectArray objects, int width, int height) {
  int pos = filename.find_last_of(".");
  std::string body = filename.substr(0, pos);
  std::string dstName = body + ".txt";
  std::ofstream writing_file;
  ...
  for (const auto& object : objects) {
    ...
    std::string writing_text =
        format("%d %f %f %f %f", object.type, x, y, w, h);
    writing_file << writing_text << std::endl;
  }
  writing_file.close();
}
...
int main(int argc, char* argv[]) {
  ...
  std::string calibType =
      get_calib_type();  // nvinfer1::CalibrationAlgoType::kMINMAX_CALIBRATION;
  ...
  cv::Rect roi(cv::Point(cropping_info.x, cropping_info.y),
               cv::Size(cropping_info.w, cropping_info.h));
  ...
  tensorrt_common::BuildConfig build_config(calibType, dla, first, last, prof,
                                            clip);
  auto trt_yolox = std::make_unique<tensorrt_yolox::TrtYoloX>(
      model_path, precision, num_class, thresh, nms_thresh, build_config, cuda,
      calibration_images, scale, "", batch_config, workspace_size);
    ...
    for (int i = 0; i < (int)filenames.size(); i = i + iter) {
      ...
      for (int b = 0; b < batch; b++) {
        if (getRandomCrop()) {
          ...
          roi.width =
              (roi.x + roi.width) > src.cols ? (src.cols - roi.x) : roi.width;
          roi.height =
              (roi.y + roi.height) > src.rows ? (src.rows - roi.y) : roi.height;
        }
        rois.push_back(roi);
      }
      ...
      for (int b = 0; b < batch; b++) {
        ...
        if (rois[b].width != -1 && rois[b].height != -1) {
          cv::rectangle(
              src, cv::Point(rois[b].x, rois[b].y),
              cv::Point(rois[b].x + rois[b].width, rois[b].y + rois[b].height),
        ...
        if (num_multitask) {
          for (int m = 0; m < num_multitask; m++) {
            ...
            if (flg_save) {
              ...
              cv::resize(cmask, resized, cv::Size(src.cols, src.rows), 0, 0,
                         cv::INTER_NEAREST);
              if (!contain(name, ".png")) {
        ...
        for (const auto& object : objects[b]) {
          ...
          if (colormap.size()) {
            ...
            cv::rectangle(src, cv::Point(left, top), cv::Point(right, bottom),
                          cv::Scalar(rgb[2], rgb[1], rgb[0]), 2);
            sprintf(buff, "%2.0f%%", object.score * 100);
            cv::putText(src, buff, cv::Point(left, top), 0, 0.5,
                        cv::Scalar(rgb[2], rgb[1], rgb[0]), 2);
          } else {
            cv::rectangle(src, cv::Point(left, top), cv::Point(right, bottom),
        ...
        if (!dont_show) {
          if (getMultiScaleInference()) {
            if (b == (batch - 1)) {
              cv::imshow("multi-scale inference image" + std::to_string(b),
                         src);
            }
          } else {
            cv::namedWindow("inference image" + std::to_string(b),
                            cv::WINDOW_NORMAL);
        ...
        if (dumpPath != "not-specified") {
          ...
          write_prediction(dumpPath, filename, names, objects[b], src.cols,
                           src.rows);
        }
        if (outputPath != "not-specified") {
          ...
          std::string filename = p.filename().string();
          std::vector<std::string> names = get_names();
          write_label(outputPath, filename, names, objects[0], src.cols,
                      src.rows);
        }
    ...
    while (1) {
      ...
      if (num_multitask) {
        for (int m = 0; m < num_multitask; m++) {
          ...
          if (cv::waitKey(1) == 'q') break;
          cv::Mat resized;
          cv::resize(cmask, resized, cv::Size(image.cols, image.rows), 0, 0,
                     cv::INTER_NEAREST);
        ...
        for (const auto& object : objects[0]) {
          ...
          if (colormap.size()) {
            ...
            cv::rectangle(image, cv::Point(left, top), cv::Point(right, bottom),
                          cv::Scalar(rgb[2], rgb[1], rgb[0]), 2);
            sprintf(buff, "%2.0f%%", object.score * 100);
            cv::putText(image, buff, cv::Point(left, top), 0, 0.5,
                        cv::Scalar(rgb[2], rgb[1], rgb[0]), 2);
          } else {
            cv::rectangle(image, cv::Point(left, top), cv::Point(right, bottom),
                          cv::Scalar(0, 0, 255), 3, 8, 2);
          }
        }
